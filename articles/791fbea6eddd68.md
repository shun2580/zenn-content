...
[CoDD（Consistency-Driven Development）](https://github.com/yohey-w/codd-dev) と組み合わせると、自然言語で話しかけるだけで要件定義→設計→実装まで自動で進みます。ただし AIは書いていないことは作りません。入力の質がそのまま出力の質になります。
