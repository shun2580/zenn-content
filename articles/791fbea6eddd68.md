...
Claude Pro には連続使用時間の制限があります。複数の Claude インスタンスを同時に動かすと、思っていた以上に早く上限に達してしまい、作業の勢いがそこで途切れてしまいます。
...
- **ローカル GPU の活用**：RTX 4060 Ti 8GB でプライバシーが気になるコードを処理
...
コスト面では、**Sonnet を使うかどうかはタスクの複雑さで判断する**のが正解でした。最初は「どうせ使うなら全員 Sonnet にすればいい」と思っていたのですが、想定より早く Claude 枠が枯渇して作業が止まったことで考えを改めました。YAML の1フィールドを書き換えるだけでも Sonnet を使うと同じ料金がかかります。単純なタスクを Haiku や Gemini に振ることで、複雑なタスクのための Claude 枠を確保できます。
